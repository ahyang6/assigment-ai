...
def _resources() -> tuple[set[str], WordNetLemmatizer]:
    ...
    for package, location in [
        ...
        try:
            ...
            # nltk.download() has no built-in timeout - on a fresh deploy
            # with no cached NLTK data, a slow or blocked connection to
            # NLTK's download servers would otherwise hang the whole app
            # indefinitely instead of failing. Bound it explicitly.
            previous_timeout = socket.getdefaulttimeout()
            socket.setdefaulttimeout(20)
            try:
                nltk.download(package, quiet=True)
            except Exception as e:
                logging.warning("Could not download NLTK package %r: %s", package, e)
            finally:
